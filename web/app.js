...
function drawBand(ctx, low, high, min, max, width, height, padding, fill) {
  if (!low || !high || low.length === 0 || high.length === 0) return;
  ctx.save();
  ctx.fillStyle = fill;
  ctx.beginPath();
  let started = false;
  for (let i = 0; i < low.length; i++) {
    ...
    const x = getXForIndex(i, low.length, width, padding);
    const y = getYForValue(v, min, max, height, padding);
    if (!started) {
      ctx.moveTo(x, y);
      started = true;
    } else {
  ...
  for (let i = high.length - 1; i >= 0; i--) {
    const v = high[i];
    if (v === null || Number.isNaN(v)) continue;
    const x = getXForIndex(i, high.length, width, padding);
    const y = getYForValue(v, min, max, height, padding);
    ctx.lineTo(x, y);
  }
  ctx.closePath();
...
async function fetchDstOutlook() {
  ...
  let data;
  try {
    const res = await fetch(apiUrl('/api/dst_outlook'), { cache: 'no-store' });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    data = await res.json();
    if (data.error) throw new Error(data.error);
  } catch (err) {
...
async function fetchSolarWindMl() {
  ...
  try {
    const steps = 7;
    const res = await fetch(apiUrl(`/api/solar_wind_ml?steps=${steps}`), { cache: 'no-store' });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    data = await res.json();
    if (data.error) throw new Error(data.error);
  } catch (err) {
...
async function fetchCmeLive() {
  try {
    let res = await fetch('/api/cme_live', { cache: 'no-store' });
    if (!res.ok && window.location.origin !== 'http://localhost:8000') {
      res = await fetch('http://localhost:8000/api/cme_live', { cache: 'no-store' });
    }
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    if (data.error) throw new Error(data.error);
...
async function fetchCmeClimo() {
  try {
    let res = await fetch('/api/cme_climatology', { cache: 'no-store' });
    if (!res.ok && window.location.origin !== 'http://localhost:8000') {
      res = await fetch('http://localhost:8000/api/cme_climatology', { cache: 'no-store' });
    }
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    if (data.error) throw new Error(data.error);
...
async function fetchCmeScenario() {
  ...
  try {
    let res = await fetch(`/api/cme_scenario?${params.toString()}`, { cache: 'no-store' });
    if (!res.ok && window.location.origin !== 'http://localhost:8000') {
      res = await fetch(`http://localhost:8000/api/cme_scenario?${params.toString()}`, { cache: 'no-store' });
    }
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    if (data.error) throw new Error(data.error);
